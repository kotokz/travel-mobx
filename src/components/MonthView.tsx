...
class ShiftRow extends React.Component<{name: string, rowdays?: MonthDate[], dataMap: ShiftMap}, {}> {
  dayToCell(date: MonthDate) {
    return <ShiftCell key={this.props.name + date.date} name={this.props.name} date={date} dataMap={this.props.dataMap} />;
  }
  render() {
    return <tr>
...
@inject("shiftStore")
@observer
class ShiftCell extends React.Component<{name: string, date: MonthDate, dataMap: ShiftMap, shiftStore?: ShiftStore}, {}> {
  eventClick(shift: Shift) {
    if (this.props.shiftStore)
      this.props.shiftStore.SetShift(this.props.name, shift, this.props.date);
  }
  buttonCell(name: string) {
    return <td className={ name }>
      <DropdownButton title={ name } id="bg-vertical-dropdown-1" className={ name } bsSize="xsmall">
  ...
  shiftToCell(shift: Shift) {
    switch (shift) {
      case Shift.Late:
        return this.buttonCell("Late");
      case Shift.Morning:
        return this.buttonCell("Morning");
      case Shift.Normal:
        return this.buttonCell("Normal");
    }
  }
  render() {
    if (!this.props.shiftStore)
    ...
    if (this.props.date.isoweekdate >= 6)
      return <td className="weekend"> { this.props.date.dayOfWeek } </td>;
    let shift = this.props.dataMap.get(this.props.date.date);
    if (shift)
